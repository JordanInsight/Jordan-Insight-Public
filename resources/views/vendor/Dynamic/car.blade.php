...
<script>
    window.routes = {
        fetchCars: "{{ route('vendor.car.fetch') }}",
        addCar: "{{ route('vendor.car.store') }}",
        editCar: "{{ route('vendor.car.edit', ['car' => 'ID_PLACEHOLDER']) }}",
        updateCar: "{{ route('vendor.car.update', ['car' => 'PLACEHOLDER']) }}",
        deleteCar: "{{ route('vendor.car.destroy', ['car' => 'PLACEHOLDER']) }}"
    };
</script>
<script src="{{ asset('vendor/scripts/car.js') }}" type="module"></script>

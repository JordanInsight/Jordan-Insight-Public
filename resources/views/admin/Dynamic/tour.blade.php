...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        window.routes = {
            fetchTours: "{{ route('admin.tour.fetch') }}",
            addTour: "{{ route('admin.tour.store') }}",
            editTour: "{{ route('admin.tour.update', '') }}/",
            showTour: "{{ route('admin.tour.show', '') }}/"
        };
        ...
        function fetchAllEntities() {
            return Promise.all([
                $.get('/admin/activities', function(response) {
                    window.activities = response.activities;
                }),
                $.get('/admin/historical-sites', function(response) {
                    window.historicalSites = response.historicalSites;
                }),
                $.get('/admin/restaurants', function(response) {
